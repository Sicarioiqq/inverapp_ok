-- Update handle_task_status_change function to ensure users are unassigned when task is completed
...
-- Fix any existing completed tasks that still have assignees
UPDATE reservation_flow_tasks
SET assignee_id = NULL
WHERE status = 'completed' AND assignee_id IS NOT NULL;

-- Delete any task assignments for completed tasks
DELETE FROM task_assignments ta
USING reservation_flow_tasks rft
WHERE ta.reservation_flow_id = rft.reservation_flow_id
AND ta.task_id = rft.task_id
AND rft.status = 'completed';