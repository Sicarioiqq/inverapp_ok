/*
  # Fix month display in commission_payment_month

  1. Changes
    - Add function to correctly format dates as month-year text
    - Add function to parse month-year text to dates
    - Ensure proper timezone handling for date display
    
  2. Notes
    - Fixes issue with month display showing incorrect month
...
CREATE OR REPLACE FUNCTION public.format_date_as_month_year(date_value date)
RETURNS text AS $$
DECLARE
  month_name text;
  year_text text;
...
CREATE OR REPLACE FUNCTION public.parse_month_year_text(month_year_text text)
RETURNS date AS $$
DECLARE
  month_name text;
  year_text text;
