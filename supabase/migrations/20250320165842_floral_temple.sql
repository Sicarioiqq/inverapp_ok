/*
  # Fix task assignments and permissions

  1. Changes
    - Add assigned_at column to reservation_flow_tasks
    - Update trigger functions to handle assignment dates
    - Fix permissions for task assignments
    - Ensure proper cleanup of assignment dates

  2. Security
    - Allow all authenticated users to assign tasks
    - Maintain admin-only restrictions for completed tasks
*/
...
-- Update task assignment policies
DROP POLICY IF EXISTS "Usuarios autenticados pueden crear asignaciones" ON task_assignments;
DROP POLICY IF EXISTS "Usuarios autenticados pueden actualizar asignaciones" ON task_assignments;
DROP POLICY IF EXISTS "Usuarios autenticados pueden eliminar asignaciones" ON task_assignments;

-- Create new policies that allow all authenticated users to manage assignments
...
BEGIN
  ...
  -- Update assigned_at based on assignments
