/*
  # Corregir estructura de tabla de reservas

  1. Cambios
    - Reordenar columnas para mejor organización
    - Corregir cálculos de campos generados
    - Agregar campos de escrituración y recuperación
...
CREATE TABLE new_reservations (
  ...
  -- Escrituración y Recuperación (UF)
  deed_payment numeric GENERATED ALWAYS AS (
    CASE 
      WHEN credit_payment > 0 THEN credit_payment
      ELSE down_payment
    END
  ) STORED,
  recovery_payment numeric GENERATED ALWAYS AS (
    reservation_payment + promise_payment + 
    CASE 
      WHEN credit_payment > 0 THEN down_payment
      ELSE 0
    END
  ) STORED,
