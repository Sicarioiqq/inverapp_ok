/*
  # Fix Operaciones task dates

  1. Changes
    - Create a function to update Solicitud Operaciones task dates based on VB Jefe Inversiones completion
    - Add trigger to automatically update dates when VB Jefe Inversiones is completed
    - Fix existing task dates for in-progress flows
    
  2. Security
    - Bypass validation checks safely using temporary trigger disabling
    - Maintain data integrity with proper error handling
*/

-- Create a function to handle VB Jefe Inversiones completion and update Solicitud Operaciones
