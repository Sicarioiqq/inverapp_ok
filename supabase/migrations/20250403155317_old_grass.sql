/*
  # Automatización de VB OC Gerencia General basada en VB OC Gerencia Comercial

  1. Nuevas Funcionalidades
    - Establecer la fecha de inicio de VB OC Gerencia General igual a la fecha de completado de VB OC Gerencia Comercial
    - Actualizar automáticamente el estado de VB OC Gerencia General a pendiente cuando VB OC Gerencia Comercial se completa
    - Mantener la sincronización de fechas entre tareas relacionadas
    
  2. Cambios
    - Crear función para manejar la lógica de automatización de VB OC Gerencia General
    - Crear trigger para detectar cambios en VB OC Gerencia Comercial
...
-- Función para manejar la automatización de VB OC Gerencia General
...
BEGIN
  ...
  IF (NEW.status = 'completed' AND OLD.status != 'completed') OR 
    ...
    -- Verificar si esta es la tarea VB OC Gerencia Comercial
    IF EXISTS (
      SELECT 1
      ...
      -- Obtener el ID de la tarea VB OC Gerencia General
      ...
      -- Obtener el ID de la tarea de flujo para VB OC Gerencia General
...
-- Crear trigger para la automatización de VB OC Gerencia General
...
DECLARE
  ...
  v_vb_oc_gerencia_completed_at timestamptz;
  v_vb_oc_gerencia_general_task_id uuid;
  v_vb_oc_gerencia_general_flow_task_id uuid;
BEGIN
  ...
  LOOP
    ...
    INTO v_vb_oc_gerencia_task_id, v_vb_oc_gerencia_flow_task_id, v_vb_oc_gerencia_completed_at
    ...
    IF v_vb_oc_gerencia_task_id IS NOT NULL AND v_vb_oc_gerencia_completed_at IS NOT NULL THEN
      -- Obtener la tarea VB OC Gerencia General
      SELECT pft.id, cft.id 
      INTO v_vb_oc_gerencia_general_task_id, v_vb_oc_gerencia_general_flow_task_id
      FROM payment_flow_tasks pft
      JOIN payment_flow_stages pfs ON pfs.id = pft.stage_id
      LEFT JOIN commission_flow_tasks cft ON cft.task_id = pft.id AND cft.commission_flow_id = v_flow.id
      WHERE pfs.flow_id = v_flow.flow_id
      ...
      IF v_vb_oc_gerencia_general_flow_task_id IS NOT NULL THEN
        -- Actualizar la tarea directamente (trigger está desactivado)
        UPDATE commission_flow_tasks
        SET started_at = v_vb_oc_gerencia_completed_at
        WHERE id = v_vb_oc_gerencia_general_flow_task_id;
      END IF;
    END IF;
  END LOOP;
