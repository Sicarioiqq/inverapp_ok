/*
  # Fix payment flow task order in Aprobación Operaciones stage

  1. Changes
    - Reorder tasks in the Aprobación Operaciones stage
    - Ensure "Solicitud Operaciones" comes before "VB Operaciones"
...
BEGIN
  ...
  -- Get the stage ID for Aprobación Operaciones
  SELECT id INTO v_stage_id 
  FROM payment_flow_stages 
  WHERE flow_id = v_flow_id 
  AND name = 'Aprobación Operaciones';
  ...
  -- Ensure "Solicitud Operaciones" comes first
  UPDATE payment_flow_tasks
  SET "order" = 1
  WHERE stage_id = v_stage_id
  AND name = 'Solicitud Operaciones';

  -- Ensure "VB Operaciones" comes second
  UPDATE payment_flow_tasks
  SET "order" = 2
  WHERE stage_id = v_stage_id
  AND name = 'VB Operaciones';

  -- If there are any other tasks in this stage, order them after these two
  UPDATE payment_flow_tasks
  SET "order" = "order" - v_temp_order + 2
  WHERE stage_id = v_stage_id
  AND name NOT IN ('Solicitud Operaciones', 'VB Operaciones')
