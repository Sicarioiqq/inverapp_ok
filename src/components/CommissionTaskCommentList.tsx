...
interface CommissionTaskCommentListProps {
  taskId: string;
  commissionFlowId: string;
  refreshTrigger?: number; // Add a refresh trigger prop
}

const CommissionTaskCommentList: React.FC<CommissionTaskCommentListProps> = ({
  taskId,
  commissionFlowId,
  refreshTrigger = 0
}) => {
  ...
  useEffect(() => {
    fetchComments();
    checkAdminStatus();
  }, [taskId, commissionFlowId, refreshTrigger]); // Add refreshTrigger to dependencies
  ...
  const fetchComments = async () => {
    try {
      ...
      const { data: flowTask, error: taskError } = await supabase
        ...
        .select('id')
        .eq('commission_flow_id', commissionFlowId)
        .eq('task_id', taskId)
        .single();
      ...
      if (flowTask) {
        ...
        const { data: commentsData, error: commentsError } = await supabase
          ...
          `)
          .eq('commission_flow_task_id', flowTask.id)
...
export default CommissionTaskCommentList;