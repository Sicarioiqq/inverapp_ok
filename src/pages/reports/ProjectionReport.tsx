...
const ProjectionReport: React.FC = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [projections, setProjections] = useState<ProjectionReportItem[]>([]);
  ...
  useEffect(() => {
    if (selectedMonth) {
      fetchProjectionData(selectedMonth);
    } else {
      setProjections([]);
    }
  }, [selectedMonth]);
  ...
  const fetchAvailableMonths = async () => {
    try {
      ...
      const { data, error } = await supabase.rpc('exec_sql', { 
        sql_query: `
          SELECT DISTINCT 
            EXTRACT(YEAR FROM commission_projection_month)::text || '-' || 
            LPAD(EXTRACT(MONTH FROM commission_projection_month)::text, 2, '0') as month_year
          FROM reservations 
          WHERE commission_projection_month IS NOT NULL 
      ...
      console.log('Meses disponibles para proyección:', data);
  ...
  const fetchProjectionData = async (month: string) => {
    try {
      setLoading(true);
      ...
      console.log('Consultando proyecciones para el período:', {
        startDate,
        endDate,
        month
      });
      ...
      console.log(`Encontradas ${data?.length || 0} proyecciones para el mes ${month}`);
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `proyeccion_comisiones_${selectedMonth}.csv`);
  ...
  const handleRowClick = (item: ProjectionReportItem) => {
    ...
    <Layout>
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-semibold text-gray-900">Proyección Comisiones</h1>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-8 text-center text-gray-500">
            {selectedMonth ? 'No hay proyecciones para el mes seleccionado' : 'Seleccione un mes para ver las proyecciones'}
          </div>
        ) : (
          <>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-medium text-gray-900">
                    Proyecciones para {formatMonthYear(selectedMonth)}
                  </h2>
                  <p className="text-sm text-gray-500">
                    Total de registros: {projections.length}
                  </p>
                </div>
