...
import { PackageSearch, AlertCircle, Loader2, Settings, Home, Package } from 'lucide-react';
...
const StockReportPage: React.FC = () => {
  ...
  const [commercialPolicies, setCommercialPolicies] = useState<ProjectCommercialPolicy[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'principal' | 'secundarios' | 'configuracion'>('principal');
  ...
  const fetchCommercialPolicies = async () => {
    try {
      ...
      setCommercialPolicies(data || []);
    } catch (err: any) {
      console.error('Error fetching commercial policies:', err);
      // Don't set the main error state to avoid blocking the stock display
    }
  };
  ...
  const renderCommercialPoliciesTable = () => (
    <div className="overflow-x-auto bg-white shadow-md rounded-lg">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-100">
          <tr>
            <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Proyecto</th>
            <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Monto Reserva (CLP)</th>
            <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Bono Pie Máx. (%)</th>
            <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Fecha Tope</th>
            <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Comuna</th>
          ...
          {commercialPolicies.length > 0 ? (
            commercialPolicies.map(policy => (
              <tr key={policy.id} className="hover:bg-gray-50 transition-colors duration-150">
                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{policy.project_name}</td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                  {policy.monto_reserva_pesos.toLocaleString('es-CL', {style: 'currency', currency: 'CLP', maximumFractionDigits: 0})}
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                  {(policy.bono_pie_max_pct * 100).toFixed(2)}%
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                  {policy.fecha_tope ? new Date(policy.fecha_tope).toLocaleDateString('es-CL') : '-'}
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{policy.comuna || '-'}</td>
                <td className="px-4 py-3 text-sm text-gray-700 max-w-md truncate">
                  {policy.observaciones || '-'}
                </td>
              </tr>
            ))
          ) : (
            <tr>
              <td colSpan={6} className="px-4 py-6 text-center text-gray-500">
                No hay políticas comerciales configuradas
              </td>
            </tr>
          )}
        </tbody>
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="bg-white shadow rounded-lg p-4 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            ...
            <select
              ...
              disabled={!selectedProject || activeTab !== 'principal'}
        ...
        <div className="bg-white shadow rounded-lg p-4 mb-6 flex flex-wrap gap-4">
          <div className="flex-1 min-w-[200px]">
            ...
            <p className="text-2xl font-semibold text-gray-800">{stockData.length}</p>
          </div>
          <div className="flex-1 min-w-[200px]">
            <p className="text-sm text-gray-500">Departamentos</p>
            <p className="text-2xl font-semibold text-blue-600">
              {stockData.filter(u => u.tipo_bien === 'DEPARTAMENTO').length}
            </p>
          </div>
          <div className="flex-1 min-w-[200px]">
            <p className="text-sm text-gray-500">Secundarios</p>
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <p className="text-2xl font-semibold text-green-600">
              {stockData.filter(u => u.estado_unidad === 'Disponible').length}
            </p>
          </div>
        </div>
          ...
          <nav className="-mb-px flex space-x-8">
            <button
              ...
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center ${
                activeTab === 'principal' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
            >
            ...
            <button
              ...
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center ${
                activeTab === 'secundarios' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
            >
            ...
            <button
              ...
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center ${
                activeTab === 'configuracion' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
            >
        ...
        {loading ? (
          ...
          <div className="p-6">
            <div className="flex items-center text-red-600 mb-4">
              ...
              <h1 className="text-2xl font-semibold">Error al Cargar Datos</h1>
            </div>
            <p className="text-red-700 bg-red-100 p-4 rounded-md">{error}</p>
          </div>
        ) : activeTab === 'principal' ? (
          <>
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Departamentos</h2>
            {renderTable(filteredPrincipales)}
          </>
        ) : activeTab === 'secundarios' ? (
          <>
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Unidades Secundarias</h2>
            {renderTable(filteredSecundarios)}
          </>
        ) : (
          <>
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Políticas Comerciales por Proyecto</h2>
            {renderCommercialPoliciesTable()}
          </>
        )}
      </div>
    </Layout>
