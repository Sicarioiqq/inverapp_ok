...
const ReservationForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si es una venta con broker, se maneja el guardado y el popup de comisión
      if (formData.is_with_broker && formData.broker_id) {
        if (id) {
          ...
          newReservationId = id;
          // NO mostrar el popup de comisión al editar
          navigate('/reservas'); 

        } else {
          // Si es una nueva reserva, crearla y mostrar el popup de comisión
          const { data, error } = await supabase
            .from('reservations')
          ...
          setLoading(false); // Detener el loading antes de mostrar el popup
          ...
          showPopup(
            <BrokerCommissionPopup
              ...
              onClose={async () => {
                ...
                if (!id) { // Asegurarse de que solo se elimine si es una nueva reserva
                  await supabase
                    .from('reservations')
                    .delete()
