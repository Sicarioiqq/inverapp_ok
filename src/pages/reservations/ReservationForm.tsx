...
const ReservationForm = () => {
  ...
  const fetchDefaultSeller = async () => {
    try {
      ...
      if (error) {
        console.error('Error fetching default seller:', error);
        throw error;
      }
      if (data) {
        setFormData(prev => ({
  ...
  const fetchReservation = async () => {
    try {
      ...
      if (reservationError) {
        ...
        console.error('Error details:', reservationError.details);
      ...
      if (!clientData) {
        throw new Error('No se encontró el cliente asociado a la reserva');
      }
  ...
  const handleClientSearch = async () => {
    ...
    try {
      ...
      console.error('Error in handleClientSearch:', err);
      setError(err.message);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Calculate total_price before applying discounts
      const total_price = formData.apartment_price + formData.parking_price + formData.storage_price;
...
// --- INICIO DE SECCIÓN MODIFICADA ---
  ...
  const createReservationFlow = async (reservationId: string, sellerId: string | null) => {
    try {
      ...
      if (flowError) {
        console.error('Error fetching sale flow:', flowError);
        throw flowError;
      }
      ...
      if (stageError) {
        console.error('Error fetching flow stage:', stageError);
        throw stageError;
      }
      ...
      if (flowRecordError) {
        console.error('Error creating reservation flow:', flowRecordError);
        throw flowRecordError;
      }
      ...
      if (sellerId) {
        ...
        if (tasksError) {
          console.error('Error fetching seller tasks:', tasksError);
          throw tasksError;
        }
        ...
        if (sellerTasks && sellerTasks.length > 0) {
          ...
          if (assignError) {
            console.error('Error creating seller task assignments:', assignError);
            throw assignError;
          }
        }
      }
      ...
      if (defaultAssignError) {
        console.error('Error fetching default task assignments:', defaultAssignError);
        throw defaultAssignError;
      }
      ...
      if (defaultAssignments && defaultAssignments.length > 0) {
        ...
        if (assignError) {
          console.error('Error creating default task assignments:', assignError);
          throw assignError;
        }
      }

    } catch (error) {
      console.error('Error in createReservationFlow:', error);
      throw error;
    }
  };
