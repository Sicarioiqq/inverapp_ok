...
const BrokerQuotePage: React.FC = () => {
  ...
  // Validate broker and load stock
  useEffect(() => {
  ...
  const filtered = stock
    .filter(u => activeTab === 'principales' ? u.tipo_bien === 'DEPARTAMENTO' : activeTab === 'secundarios' ? u.tipo_bien !== 'DEPARTAMENTO' : true)
    .filter(u => filterProyecto === 'Todos' || u.proyecto_nombre === filterProyecto)
      ...
      <main className="container mx-auto p-4">
        ...
        {(activeTab==='principales' || activeTab==='secundarios') && (
          <div className="flex space-x-4 mb-4">
            <select value={filterProyecto} onChange={e=>{setFilterProyecto(e.target.value); setFilterTipologia('Todos');}} className="border p-2 rounded">
              <option>Todos</option>
              {proyectos.map(p=> <option key={p}>{p}</option>)}
            </select>
            <select value={filterTipologia} onChange={e=>setFilterTipologia(e.target.value)} className="border p-2 rounded">
              <option>Todos</option>
          ...
          <table className="min-w-full">
            <thead className="bg-gray-200">
              <tr>
                {(activeTab==='principales'? headersPrincipales: headersSecundarios).map(h=> (
                  <th key={h.key} className="px-4 py-2 text-left cursor-pointer" onClick={()=>{
                    if(sortField===h.key) setSortAsc(!sortAsc); else { setSortField(h.key); setSortAsc(true);}  }}>
            ...
            <tbody>
              {loadingStock ? (
                ...
                return (
                  <tr key={u.id} className="border-t hover:bg-gray-50 cursor-pointer" onClick={()=>setSelectedUnidad(u)}>
          ...
          <div className="bg-white shadow rounded p-6 mt-6 space-y-8">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={cliente} onChange={e=>setCliente(e.target.value)} placeholder="Nombre del Cliente" className="mt-1 block w-full border border-blue-300 rounded-md bg-white p-2 focus:ring-blue-500 focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">RUT</label>
                <input type="text" value={rut} onChange={e=>setRut(e.target.value)} placeholder="RUT del Cliente" className="mt-1 block w-full border border-blue-300 rounded-md bg-white p-2 focus:ring-blue-500 focus:border-blue-500" />
              </div>
            </section>
            ...
            <section>
              ...
              <p className="mt-1 text-gray-600">{selectedUnidad?.unidad||'-'} <span className="text-sm text-gray-500">({selectedUnidad?.estado_unidad||'-'})</span></p>
            ...
            <section>
              ...
              <p className="mt-1 text-gray-600">Sup. Útil: <span className="font-semibold">{selectedUnidad?.sup_util!=null?selectedUnidad.sup_util.toLocaleString(undefined,{minimumFractionDigits:2})+' m²':'-'}</span></p>
            </section>
          </div>
        )}
      </main>
