...
interface Unidad {
  ...
  fecha_carga?: string; // Add fecha_carga to Unidad interface
}
...
const BrokerQuotePage: React.FC = () => {
  ...
  const [stockLoadDate, setStockLoadDate] = useState<string | null>(null); // New state for load date
  ...
  // Cargar TODO el stock con paginación de 1000 en 1000 y la fecha de carga
  useEffect(() => {
    if (!brokerInfo) return;
    const fetchStockAndDate = async () => {
      setLoadingStock(true);
      try {
        const pageSize = 1000;
        let from = 0;
        const all: Unidad[] = [];
        let latestLoadDate: string | null = null;
        ...
        while (true) {
          ...
          const { data, error: se } = await supabase
            ...
            .select(
              'id, proyecto_nombre, unidad, tipologia, piso, sup_util, sup_terraza, sup_total, valor_lista, descuento, estado_unidad, tipo_bien, fecha_carga'
            )
            .order('fecha_carga', { ascending: false }) // Order by fecha_carga descending
            .range(from, to);

          if (se) throw se;
          if (data && data.length) {
            all.push(...data);
            // Capture the latest fecha_carga from the first record if it exists
            if (from === 0 && data[0]?.fecha_carga) {
              latestLoadDate = data[0].fecha_carga;
            }
        ...
        setStock(all);
        setStockLoadDate(latestLoadDate); // Set the latest load date
      } catch (e) {
    ...
    fetchStockAndDate(); // Call the updated fetch function
  ...
  if (isValidating || loadingCommissions || loadingUf || loadingStock) // Also wait for stock to load
    return (
