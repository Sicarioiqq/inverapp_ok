...
const BrokerQuotePage: React.FC = () => {
    ...
    useEffect(() => {
        const validateBroker = async () => {
            ...
            try {
                ...
                setError("Acceso no autorizado o token inválido");
                setIsValidating(false);
            }
        };
    ...
    const proyectos = useMemo(() => {
        ...
        return ['Todos', ...uniqueProyectos];
    }, [stock]);

    // Get unique tipologias for filter
    const tipologias = useMemo(() => {
        let uniqueTipologias: string[] = [];
        if (filterProyecto !== 'Todos') {
            uniqueTipologias = Array.from(
                new Set(
                    stock
        ...
        return ['Todos', ...uniqueTipologias];
    }, [stock, filterProyecto]);

    // Sort the filtered stock
    ...
    const handleSelectUnit = (unit: Unidad) => {
        ...
        // Reset secondary units when changing main unit
        setAddedSecondaryUnits([]);
        
        // Find secondary units for this project
        const secondaryUnits = stock.filter(
            u => u.proyecto_nombre === unit.proyecto_nombre && 
    ...
    const handleAddSecondaryUnit = () => {
        ...
        const unitToAdd = projectSecondaryUnits.find(u => u.id === selectedSecondaryUnitToAdd);
        if (!unitToAdd) return;
        
        // Check if already added
        if (addedSecondaryUnits.some(u => u.id === unitToAdd.id)) {
    ...
    // Handle removing an added secondary unit
    const handleRemoveAddedSecondaryUnit = (unitId: string) => {
        setAddedSecondaryUnits(prev => prev.filter(u => u.id !== unitId));
    };
    ...
    // Calculated values for the quotation
    const precioBaseDepartamento = selectedUnidad?.valor_lista || 0;
    
    // Calculate discount amount in UF
        ...
        <div className="min-h-screen bg-gray-100">
            ...
            <main className="container mx-auto p-4">
                                        ...
                                        <tr>
                                            ...
                                            >
                                                ...
                                                {sortField === 'proyecto_nombre' && (
                                                    sortDirection ? <ArrowUp className="inline-block h-4 w-4 ml-1" /> : <ArrowDown className="inline-block h-4 w-4 ml-1" />
                                                )}
                                            </th>
                                            <th 
                                                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
                                            ...
                                            .map(unit => (
                                                <tr key={unit.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                                                        {unit.sup_util?.toFixed(2)} m²
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                                        {unit.valor_lista?.toLocaleString()} UF
                                                    </td>
                ...
                {activeTab === 'secundarios' && selectedUnidad && (
                    <>
                        <div className="bg-white shadow rounded-lg p-6 mb-6">
                            <h2 className="text-xl font-semibold mb-4">Unidades Secundarias Disponibles</h2>
                            <div className="flex flex-col md:flex-row gap-4 items-end">
                                <div className="flex-1">
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Seleccionar Unidad Secundaria</label>
                                    <select
                                ...
                                >
                                    <PlusCircle className="h-5 w-5 inline-block mr-1" /> Agregar
                                </button>
                            </div>
                        </div>
                        ...
                        {addedSecondaryUnits.length > 0 && (
                            <div className="bg-white shadow rounded-lg overflow-hidden">
                                <div className="p-6 border-b">
                                    <h2 className="text-xl font-semibold">Unidades Secundarias Seleccionadas</h2>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                            ...
                            <div className="bg-white shadow rounded p-6 mt-6">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700">Precio Base Departamento</label>
                                                <div className="mt-1 flex justify-between items-center bg-gray-50 p-2 rounded">
                                                    <span>{formatCurrency(precioBaseDepartamento)} UF</span>
                                                    <span className="text-gray-500">{ufToPesos(precioBaseDepartamento)}</span>
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700">Descuento ({discountAmount.toFixed(2)}%)</label>
                                                <div className="mt-1 flex justify-between items-center bg-gray-50 p-2 rounded">
                                                    <span>{formatCurrency(precioDescuentoDepartamento)} UF</span>
                                                    <span className="text-gray-500">{ufToPesos(precioDescuentoDepartamento)}</span>
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700">Precio Departamento con Descuento</label>
                                                <div className="mt-1 flex justify-between items-center bg-gray-50 p-2 rounded">
                                                    <span className="font-semibold">{formatCurrency(precioDepartamentoConDescuento)} UF</span>
                                                    <span className="text-gray-500">{ufToPesos(precioDepartamentoConDescuento)}</span>
                                                </div>
                                            </div>
                                            {addedSecondaryUnits.length > 0 && (
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700">Precio Total Secundarios</label>
                                                    <div className="mt-1 flex justify-between items-center bg-gray-50 p-2 rounded">
                                                        <span>{formatCurrency(precioTotalSecundarios)} UF</span>
                                                        <span className="text-gray-500">{ufToPesos(precioTotalSecundarios)}</span>
                                                    </div>
                                                </div>
                                            )}
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700">Total Escrituración</label>
                                                <div className="mt-1 flex justify-between items-center bg-blue-50 p-2 rounded">
                                                    <span className="font-semibold text-blue-800">{formatCurrency(totalEscritura)} UF</span>
                                                    <span className="text-blue-600">{ufToPesos(totalEscritura)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                <div className="mt-1 flex justify-between items-center bg-gray-50 p-2 rounded">
                                                    ...
                                                    <span className="text-gray-500">{ufToPesos(pagoReserva)}</span>
                                                </div>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700">Promesa (UF)</label>
                                                    <input
                                                        type="number"
                                                        value={pagoPromesa}
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700">Pie (UF)</label>
                                                    <input
                                                        type="number"
                                                        value={pagoPie}
                                            ...
                                            {quotationType !== 'descuento' && (
                                                <div>
                                                    ...
                                                    <div className="mt-1 flex justify-between items-center bg-green-50 p-2 rounded">
                                                        <span className="font-semibold text-green-800">{formatCurrency(pagoBonoPieCotizacion)} UF</span>
                                                        <span className="text-green-600">{ufToPesos(pagoBonoPieCotizacion)}</span>
                                                    </div>
                                                </div>
                                            )}
                                            <div>
                                                ...
                                                <div className="mt-1 flex justify-between items-center bg-gray-50 p-2 rounded">
                                                    ...
                                                    <span className="text-gray-500">{ufToPesos(pagoCreditoHipotecarioCalculado)}</span>
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700">Total Forma de Pago</label>
                                                <div className="mt-1 flex justify-between items-center bg-blue-50 p-2 rounded">
                                                    <span className="font-semibold text-blue-800">{formatCurrency(totalFormaDePago)} UF</span>
                                                    <span className="text-blue-600">{ufToPesos(totalFormaDePago)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
