...
import { supabase, formatDateChile, formatDateTimeChile, formatCurrency } from '../../lib/supabase'; // Asegúrate que formatCurrency se exporte desde aquí
...
import { differenceInDays, addDays } from 'date-fns'; // No necesitas es from date-fns/locale si formatDateChile ya lo maneja
...
interface Task {
  ...
  comments_count: number; isCollapsed?: boolean; // isCollapsed para la UI
}
interface Stage {
...
interface AtRiskPopupProps { // Definida aquí si no está importada
  commissionId: string; isAtRisk: boolean; reason: string | null;
  onSave: () => void; onClose: () => void;
}
// --- FIN: Definiciones de Tipos ---

const retryOperation = async (operation: () => Promise<any>, maxRetries: number = 3, initialDelay: number = 1000): Promise<any> => { /* ... (tu código existente) ... */ };
...
const PaymentFlowPage: React.FC = () => {
  ...
  useEffect(() => { // Efecto principal para cargar datos del flujo
    if (id) {
      setLoading(true); setError(null); 
      Promise.all([ fetchFlow(), fetchUsers(), checkAdminStatus() ])
        .catch((err) => { console.error("Error en carga inicial:", err); setError(err.message || "Error al cargar datos del flujo."); })
        .finally(() => { setLoading(false); });
    } else { navigate('/pagos'); }
  }, [id, commentRefreshTrigger]);

  useEffect(() => { // Efecto para cargar datos dependientes (promociones, resumen PDF)
    const loadDependentData = async () => {
      if (flow && flow.broker_commission && flow.broker_commission.reservation && flow.broker_commission.reservation.id) {
        ...
        try {
          const { data: promos, error: promoError } = await supabase.from('promotions').select('*').eq('reservation_id', reservationId).order('created_at', { ascending: true });
          ...
          if (reservationData.total_payment !== undefined && reservationData.subsidy_payment !== undefined &&
              reservationData.minimum_price !== undefined && commissionData.commission_amount !== undefined) {
          ...
          console.error("Error cargando datos para PDF:", err);
          setError(prev => prev ? `${prev}\nError PDF: ${err.message}` : `Error PDF: ${err.message}`);
          setAppliedPromotions([]); setFinancialSummaryForPDF(null);
        } finally { setLoadingPdfData(false); }
      } else if (flow) { setLoadingPdfData(false); setAppliedPromotions([]); setFinancialSummaryForPDF(null); }
    };
    if (flow) { loadDependentData(); }
  }, [flow]); 

  const checkAdminStatus = async () => { /* ... (tu código sin cambios) ... */ };
  const fetchUsers = async () => { /* ... (tu código sin cambios) ... */ };
  
  const fetchFlow = async () => { // Asegúrate que esta función setea flow a null en caso de error
    try {
      const { data: flowData, error: flowError } = await supabase.from('commission_flows')
        .select(`id, status, started_at, completed_at, is_second_payment, flow:payment_flows(id), current_stage:payment_flow_stages(id, name), broker_commission:broker_commissions(id, commission_amount, number_of_payments, first_payment_percentage, at_risk, at_risk_reason, reservation:reservations(id, reservation_number, apartment_number, total_payment, subsidy_payment, minimum_price, is_rescinded, rescinded_at, rescinded_reason, client:clients(id, first_name, last_name, rut), project:projects(name, stage), broker:brokers(id, name, business_name)))`)
        .eq('id', id).single();
      if (flowError) throw flowError;
      if (!flowData) { setFlow(null); throw new Error(`Flujo de comisión ID ${id} no encontrado.`);}
      if (!flowData.broker_commission) { setFlow(null); throw new Error(`Flujo ${flowData.id} sin comisión.`); }
      if (!flowData.broker_commission.reservation) { setFlow(null); throw new Error(`Comisión para flujo ${flowData.id} sin reserva.`); }

      const { data: stagesData, error: stagesError } = await supabase.from('payment_flow_stages').select(`id, name, order, tasks:payment_flow_tasks(id, name, days_to_complete, default_assignee:profiles(id, first_name, last_name, avatar_url))`).eq('flow_id', flowData.flow.id).order('order', { ascending: true });
      if (stagesError) throw stagesError;

      const { data: flowTasks, error: tasksError } = await supabase.from('commission_flow_tasks').select(`id, task_id, status, started_at, completed_at, assigned_at, assignee:profiles(id, first_name, last_name, avatar_url)`).eq('commission_flow_id', id);
      ...
      if (commissionFlowTaskIds.length > 0) {
        const { data: fetchedCommentsData, error: commentsFetchError } = await supabase.from('commission_task_comments').select('commission_flow_task_id').in('commission_flow_task_id', commissionFlowTaskIds);
    ...
    } catch (err) { setFlow(null); throw err; }
  };
  ...
  const navigateToEditClient = () => { /* ... (código como lo tienes) ... */ };
  const navigateToEditReservation = () => { /* ... (código como lo tienes) ... */ };
  const navigateToEditCommission = () => { /* ... (código como lo tienes) ... */ };
  const navigateToReservationFlow = async () => { /* ... (código como lo tienes) ... */ };
  ...
  const AtRiskPopup: React.FC<AtRiskPopupProps> = ({ commissionId, isAtRisk, reason, onSave, onClose }) => {
    ...
    return ( // --- MODIFICACIÓN: Eliminado el comentario problemático aquí ---
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto pb-12">
        {/* ... (Header y Tarjeta de Información General sin cambios significativos en su estructura, solo usan reservationDetails y commissionDetails) ... */}
        
        {/* --- NUEVA TARJETA: Informes y Documentos --- */}
        <div className="bg-white p-6 rounded-lg shadow-md mt-6"> {/* Añadido mt-6 para separación */}
        ...
        <div className="space-y-6 mt-6"> {/* Añadido mt-6 para separación */}
              ...
              <div id={`stage-${stage.id}`} key={stage.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                  ...
                  {stage.isExpanded && (
                      <div className="divide-y divide-gray-200">
                          {stage.tasks.map((task, taskIndexInStage) => { // Renombrado taskIndex a taskIndexInStage para evitar conflicto con variable externa si existiera
                              ...
                              <div key={task.id} className={`p-6 ${showCollapsedView ? 'py-3' : 'hover:bg-gray-50'}`}>
                                  {showCollapsedView ? (
                                      <div className="flex items-center justify-between">
                                          ...
                                          <button onClick={() => toggleTaskCollapse(stageIndex, taskIndexInStage)} className="p-1 text-blue-600 hover:text-blue-800 rounded-md hover:bg-blue-50" title="Expandir tarea"><ChevronRight className="h-5 w-5" /></button>
                                      </div>
                                  ) : (
                                      <>
                                        <div className="flex items-center justify-between">
                                          <div className="flex-1">
                                            <div className="flex items-center justify-between mb-2">
                                              ...
                                              {isTaskCompleted ? (<button onClick={() => toggleTaskCollapse(stageIndex, taskIndexInStage)} className="p-1 text-blue-600 hover:text-blue-800 rounded-md hover:bg-blue-50" title="Colapsar tarea"><ChevronDown className="h-5 w-5" /></button>) 
                                              : (
...
// Asegúrate que AtRiskPopupProps esté definida si AtRiskPopup está en este archivo
