...
import {
  ...
  Edit,
  ChevronDown,
  ChevronRight, // Asegúrate que ChevronRight esté importado
  Edit2,
  Users,
  ListChecks,
  FileText,
...
const PaymentFlowComponent: React.FC = () => { // Renombrado para evitar conflicto con la interfaz PaymentFlow
  const { id } = useParams();
  ...
  const [editingTaskDate, setEditingTaskDate] = useState<{
    taskId: string; // Este será el ID de la plantilla de tarea
    type: 'start' | 'complete';
  } | null>(null);
  ...
  }, [id, commentRefreshTrigger]); // commentRefreshTrigger añadido para recargar el flujo si es necesario
  ...
  const fetchFlow = async () => {
    try {
      ...
      if (commissionFlowTaskIds.length > 0) {
        const { data: fetchedCommentsData, error: commentsError } = await supabase
          .from('commission_task_comments')
          .select('commission_flow_task_id')
          .in('commission_flow_task_id', commissionFlowTaskIds); // Filtrar comentarios
      ...
      const stages = stagesData?.map(stage => {
        const stageTasks = stage.tasks.map(taskTemplate => { // Renombrado a taskTemplate para claridad
          ...
          return {
            id: taskTemplate.id, // ID de la plantilla de tarea
            commission_flow_task_id: flowTaskInstance?.id, // ID de la instancia de tarea en este flujo
            ...
            isCollapsed: isCompleted, // MODIFICACIÓN: Colapsar si está completada por defecto
          };
        });
  ...
  // ... (handleStartFlow, handleStartDateChange, handleTaskDateChange sin cambios)
  ...
  const handleTaskDateChange = async (templateTaskId: string, date: string, type: 'start' | 'complete') => {
    ...
    try {
      ...
      const { data: flowTask, error: taskError } = await supabase
        ...
        .eq('task_id', templateTaskId) // usa templateTaskId para buscar
        .maybeSingle();
      ...
      if (flowTask) {
        await retryOperation(async () => {
          const { error: updateError } = await supabase
            .from('commission_flow_tasks')
            .update({ [type === 'start' ? 'started_at' : 'completed_at']: date })
            .eq('id', flowTask.id); // Actualiza usando el ID de la instancia
          if (updateError) throw updateError;
        });
      }
      await fetchFlow();
  ...
  const handleAssign = async (templateTaskId: string, currentAssignee: User | null, defaultAssignee: User | null) => {
    ...
    try {
      if (!currentAssignee && defaultAssignee) {
        await assignUser(templateTaskId, defaultAssignee);
        return;
      }
        ...
        showPopup(
          <div className="space-y-4">
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t">
              ...
              {currentAssignee && (
                <button type="button" onClick={() => resolve({ id: '', first_name: '', last_name: '', position: '' })} className="px-4 py-2 text-sm font-medium text-red-700 bg-white border border-red-300 rounded-md shadow-sm hover:bg-red-50">Quitar Asignación</button>
              )}
            </div>
          </div>,
    ...
    try {
      ...
      if (flowTask) {
        await retryOperation(async () => {
          const { error: updateError } = await supabase
            .from('commission_flow_tasks')
            .update({ assignee_id: user.id || null, assigned_at: user.id ? new Date().toISOString() : null })
            .eq('id', flowTask.id); // Actualiza usando el ID de la instancia
          if (updateError) throw updateError;
        });
      } else {
        await retryOperation(async () => {
          const { error: createError } = await supabase
            ...
            .insert({
              ...
              task_id: templateTaskId, // usa templateTaskId para crear
              status: 'pending',
    ...
    if (flow?.status === 'in_progress') {
      flow.stages.forEach(stage =>
        stage.tasks.forEach(task => {
          // task.id aquí es el ID de la plantilla
  ...
  }, [flow?.status, flow?.stages]); // Dependencia más granular
  ...
  const handleStatusChange = async (templateTaskId: string, newStatus: string) => {
    ...
    try {
      ...
      if (newStatus === 'completed' && flowTask) {
        updateData.completed_at = new Date().toISOString();
        // Si no hay fecha de inicio y se completa, se puede poner la fecha actual como inicio
        const {data: currentTaskData} = await supabase.from('commission_flow_tasks').select('started_at').eq('id', flowTask.id).single();
        if(!currentTaskData?.started_at) {
            updateData.started_at = new Date().toISOString();
        }

      } else if (newStatus === 'in_progress' && flowTask) {
         // Si no hay fecha de inicio y se pone en progreso, se puede poner la fecha actual como inicio
         const {data: currentTaskData} = await supabase.from('commission_flow_tasks').select('started_at').eq('id', flowTask.id).single();
         if(!currentTaskData?.started_at) {
             updateData.started_at = new Date().toISOString();
         }
      ...
      if (flowTask) {
        ...
        // Solo insertar si la tarea no existe, el status por defecto será el nuevo status
  ...
  const handleAddComment = async (templateOrInstanceId: string, isInstanceId: boolean = false) => {
    if (!flow || flow.status === 'pending') return;
    let taskInstanceId = templateOrInstanceId;

    if (!isInstanceId) {
        // Si es ID de plantilla, buscar el ID de instancia
        const { data: flowTask, error: findError } = await supabase
            .from('commission_flow_tasks')
            .select('id')
            .eq('commission_flow_id', flow.id)
            .eq('task_id', templateOrInstanceId)
            .maybeSingle();
        if (findError || !flowTask) {
            setError("No se pudo encontrar la tarea para comentar.");
            console.error("Error finding task for comment or task not found:", findError);
            return;
        }
        taskInstanceId = flowTask.id;
    }
    
    showPopup(
      <CommissionTaskCommentPopup
        taskId={taskInstanceId} // Usar el ID de la instancia de la tarea
        commissionFlowId={flow.id}
        onSave={() => { fetchFlow(); setCommentRefreshTrigger(prev => prev + 1); }}
  ...
  const toggleTaskComments = (templateOrInstanceId: string, isInstanceId: boolean = false) => {
    // Si se colapsa/expande una tarea, el ID para los comentarios debe ser el de la instancia.
    // Si `templateOrInstanceId` ya es el de la instancia (cuando se hace clic desde la sección de comentarios), se usa directamente.
    // Si es el de la plantilla (cuando se hace clic en el botón "Ver comentarios" general de la tarea), se necesita el de la instancia.
    // Por ahora, `expandedTaskId` almacenará el ID que se le pase.
    // `CommissionTaskCommentList` necesitará el ID de la instancia.
    setExpandedTaskId(expandedTaskId === templateOrInstanceId ? null : templateOrInstanceId);
    if (expandedTaskId !== templateOrInstanceId) {
      setCommentRefreshTrigger(prev => prev + 1);
    }
  };

  // NUEVA FUNCIÓN: Para alternar el estado colapsado de una tarea
  ...
  // ... (getStatusIcon, getStatusText, getStatusColor, calculateProgress sin cambios)
  ...
  const getDaysOverdue = (task: Task) => {
    if (task.status === 'completed' || !task.started_at || !task.days_to_complete) return 0; // No hay retraso si está completada
    const endDateToCompare = new Date(); // Comparar siempre con la fecha actual para tareas no completadas
  ...
  // ... (formatDate, formatDateTime, funciones de navegación sin cambios)
  ...
  const handleCreateSecondPaymentFlow = async () => {
    ...
    try {
      ...
      if (flowError || !flowData || flowData.stages.length === 0) throw flowError || new Error("Flujo de segundo pago o sus etapas no configurados.");
      
      // Asegurar que las etapas están ordenadas y tomar la primera
      const firstStage = flowData.stages.sort((a, b) => a.order - b.order)[0];
  ...
  const handleToggleAtRisk = () => {
    if (!flow) return;
    showPopup(
      <AtRiskPopup commissionId={flow.broker_commission.id} isAtRisk={flow.broker_commission.at_risk || false} reason={flow.broker_commission.at_risk_reason || ''} onSave={fetchFlow} onClose={() => { /* showPopup(null) si es necesario */ }} />,
  ...
  if (error || !flow) {
    return <Layout><div className="bg-red-50 text-red-600 p-4 rounded-lg">{error || 'Flujo no encontrado'}</div></Layout>;
  }
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto">
        {/* ... (Cabecera y Navegación sin cambios) ... */}
            ...
            <div id={`stage-${stage.id}`} key={stage.id} className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              {stage.isExpanded && (
                <div className="divide-y divide-gray-200">
                  {stage.tasks.map((task, taskIndex) => { // Añadido taskIndex
                    const isTaskCompleted = task.status === 'completed';
                    // MODIFICACIÓN: Usar task.isCollapsed para determinar si mostrar la vista colapsada
                    ...
                    // const expectedDate = getExpectedDate(task); // No se usa en el JSX directamente
                    // const hasComments = task.comments_count > 0; // No se usa en el JSX directamente

                    return (
                      <div key={task.id} className={`p-6 ${!showCollapsedView ? 'hover:bg-gray-50' : ''}`}>
                        {showCollapsedView ? (
                          // ------ MODIFICACIÓN: VISTA COLAPSADA ------
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              <CheckCircle2 className="h-5 w-5 text-green-600 mr-3 flex-shrink-0" />
                              <h4 className="text-base font-medium text-gray-700 line-through">
                                {task.name}
                              </h4>
                            </div>
                            <button
                          ...
                          // ------ MODIFICACIÓN: VISTA EXPANDIDA (EXISTENTE CON AJUSTES) ------
                          <>
                            <div className="flex items-center justify-between"> {/* Contenedor principal de la tarea expandida */}
                              <div className="flex-1"> {/* Contenedor para título y detalles, permite que el botón de colapsar/status esté a la derecha */}
                                <div className="flex items-center justify-between mb-2">
                                  <h4 className={`text-base font-medium ${isTaskCompleted ? 'text-gray-700 line-through' : 'text-gray-900'}`}>
                                    {task.name}
                                  </h4>
                                  {isTaskCompleted ? ( // Si está completada y expandida, mostrar botón para colapsar
                                    <button
                                      onClick={() => toggleTaskCollapse(stageIndex, taskIndex)}
                                      className="p-1 text-blue-600 hover:text-blue-800 rounded-md hover:bg-blue-50"
                                  ...
                                  ) : ( // Si no está completada, mostrar el selector de estado
                                    <select
                                      value={task.status}
                                ...
                                <div className="flex items-center space-x-4 text-sm text-gray-500">
                                  ...
                                  <button onClick={() => handleAddComment(task.commission_flow_task_id || task.id, !!task.commission_flow_task_id)} className="flex items-center text-gray-500 hover:text-gray-700 relative" disabled={flow.status === 'pending'}>
                                  ...
                                  <button onClick={() => toggleTaskComments(task.commission_flow_task_id || task.id, !!task.commission_flow_task_id)} className="flex items-center text-gray-500 hover:text-gray-700" disabled={flow.status === 'pending' || task.comments_count === 0}>
                                    <span>Ver comentarios</span> <ChevronDown className={`h-4 w-4 ml-1 transition-transform ${expandedTaskId === (task.commission_flow_task_id || task.id) ? 'rotate-180' : ''}`} />
                                  </button>
                                </div>
                                    ...
                                    <div className="flex flex-col space-y-2">
                                      <div className="flex items-center">
                                        {editingTaskDate && editingTaskDate.taskId === task.id && editingTaskDate.type === 'start' ? (
                                          <input type="datetime-local" value={tempDateValue} onChange={(e) => handleDateInputChange(e, task.id, 'start')} className="text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"/>
                                        ) : (
                                      ...
                                      {task.completed_at && (
                                        <div className="flex items-center">
                                          {editingTaskDate && editingTaskDate.taskId === task.id && editingTaskDate.type === 'complete' ? (
                                            <input type="datetime-local" value={tempDateValue} onChange={(e) => handleDateInputChange(e, task.id, 'complete')} className="text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"/>
                                          ) : (
                            ...
                            {expandedTaskId === (task.commission_flow_task_id || task.id) && task.comments_count > 0 && (
                              <div className="mt-4 pt-4 border-t border-gray-200">
                                <CommissionTaskCommentList
                                  taskId={task.commission_flow_task_id!} // Usar el ID de instancia aquí, asegurando que no es undefined
                                  commissionFlowId={flow.id}
...
// Componente AtRiskPopup (sin cambios, pero lo incluyo para que el archivo esté completo)
...
const AtRiskPopup: React.FC<AtRiskPopupProps> = ({ commissionId, isAtRisk, reason, onSave, onClose }) => {
  const { hidePopup } = usePopup(); // Asegúrate que usePopup está disponible o pásalo como prop si es necesario
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({ at_risk: atRisk, at_risk_reason: atRisk ? atRiskReason : null })
        .eq('id', commissionId);
      if (updateError) throw updateError;
      hidePopup(); // Asegúrate que hidePopup se llame
      onSave();
    } catch (err: any) {
      setError(err.message);
    } finally {
...
export default PaymentFlowComponent; // Renombrado para exportar